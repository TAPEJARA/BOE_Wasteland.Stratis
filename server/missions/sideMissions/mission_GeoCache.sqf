...
_setupObjects =
{
	...
	_geoPos = _missionPos vectorAdd ([[20 + random 20, 0, 0], random 360] call BIS_fnc_rotateVector2D);	
	_geoCache = createVehicle ["Land_SurvivalRadio_F",[(_geoPos select 0), (_geoPos select 1),0],[], 0, "NONE"];

	_missionHintText = "Uma GeoCache foi marcada no mapa. Há um pequeno objeto escondido perto do marcador. Encontre-o e uma recompensa será entregue por via aérea!";
};
...
_successExec =
{
	...
	_successHintMessage = "Os suprimentos da GeoCache foram entregues pelo pára-quedas!";
};

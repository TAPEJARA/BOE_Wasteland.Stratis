...
// Timers
APOC_coolDownTimer = 900;         // APOC Air Drop Cool Down Timer
BoS_coolDownTimer = 1800;           // Baselocker hacking timer
Safe_coolDownTimer = 1800;          // Safe hacking timer
A3W_teamSwitchLock = 3600;          // Time in seconds before a player is locked into the team he is playing in (INDEPENDENT, BLUFOR, OPFOR)

// Store NPC additions
Sell_Price = 1.5;                    // Devider for the store price (e.g. a vehicle that is $10.000 in the store will sell for $5.000 when set to 2)
...
Service_PriceService_Price = 8;                 // Devider for the store price (e.g. a vehicle that is $10.000 in the store will service for $2.500 when set to 4)
...
A3W_timeMultiplierDay = 4.1;       // Sets the speed of time between 5 AM and 8 PM (for example, 6.0 means 6 hours in-game will pass in 1 real hour)
A3W_timeMultiplierNight = 19.33;     // Sets the speed of time between 8 PM and 5 AM
A3W_moonLight = 1;                 // Moon light during night (0 = no, 1 = yes)

// Player settings
A3W_startingMoney = 1500000;           // Amount of money that players start with
...
// Spawn settings
A3W_townSpawnCooldown = 60;      // Number of seconds to wait between each spawn on a specific town (0 = disabled)
A3W_townSpawnSpawnHeight = 0;   // Altitude in meters at which players will spawn when spawning on a specific town (0 = ground/sea)
A3W_spawnBeaconCooldown = 3*60;    // Number of seconds to wait between each use of a specific spawn beacon (0 = disabled)
A3W_spawnBeaconSpawnHeight = 1298; // Altitude in meters at which players will spawn when using spawn beacons (0 = ground/sea)
...
A3W_atmMaxBalance = 6000000;       // Maximum amount of money that can be stored in a bank account (don't go over 16777216 as numbers start losing accuracy)
...
A3W_moneySaving = 0;               // If playerSaving = 1, save player money amount (0 = no, 1 = yes)
...
A3W_objectLifetime = 40*24;         // Maximum lifetime in hours for saved objects (baseparts, crates, etc. except vehicles) across server restarts (0 = no time limit)
A3W_vehicleLifetime = 20*24;           // Maximum lifetime in hours for saved vehicles across server restarts, regardless of usage (0 = no time limit)
...
A3W_vehicleQuantity = 80;         // Approximate number of land vehicles to be spawned in towns
A3W_boatSpawning = 0;              // If serverSpawning = 1, spawn boats at marked areas near coasts (0 = no, 1 = yes)
...
A3W_missionsQuantity = 3;          // Number of missions running at the same time (0 to 6)
A3W_heliPatrolMissions = 1;        // Enable missions involving flying helicopters piloted by AI (0 = no, 1 = yes)
A3W_underWaterMissions = 1;        // Enable underwater missions which require diving gear (0 = no, 1 = yes)
A3W_mainMissionDelay = 6*60;      // Time in seconds between Main Missions
A3W_mainMissionTimeout = 120*60;    // Time in seconds that a Main Mission will run for, unless completed
A3W_sideMissionDelay = 3*60;       // Time in seconds between Side Missions
A3W_sideMissionTimeout = 120*60;    // Time in seconds that a Side Mission will run for, unless completed
A3W_moneyMissionDelay = 9*60;     // Time in seconds between Money Missions

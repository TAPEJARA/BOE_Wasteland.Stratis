...
FAR_public_EH =
{
	...
	if (_EH == "FAR_deathMessage") then
	{
		...
		if (alive _unit) then
		{
			switch (true) do
			{
				case (isNil "_killerName"): 
				{ 
				 systemChat "=======================================";
				 systemChat format ["%1 foi FERIDO", toString _unitName]; 
				};
				case (!isNil "_killerName" && !isPlayer _killer): 
				{ 
				 systemChat "=======================================";
				 systemChat format ["%1 foi FERIDO por um BOT", toString _unitName]; 
				};
				default 
				{
				 systemChat "=======================================";
				 systemChat format ["%1 foi FERIDO por %2", toString _unitName, toString _killerName]; 
				};
			};
		};
	};

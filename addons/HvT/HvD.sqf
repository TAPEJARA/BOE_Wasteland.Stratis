...
for "_i" from 0 to 1 step 0 do 
{
	...
	if (isNil "createDrugsMarker" && _lsd > 3 || isNil "createDrugsMarker" && _mar > 3 || isNil "createDrugsMarker" && _coc > 3 || isNil "createDrugsMarker" && _her > 3) then
		{
			_title  = "<t color='#ff0000' size='1.2' align='center'>Traficante! </t><br />";
			_name = format ["%1<br /> ",name player];     
			_text = "<t color='#FFFFFF' shadow='1' shadowColor='#000000' align='center'>Alguém viu você transportanto drogas e marcou sua localização no mapa!</t><br />";     
			...
			createDrugsMarker = 
			{
				...
				_drugMarker setMarkerText (format ["Traficante: %1", name player]);

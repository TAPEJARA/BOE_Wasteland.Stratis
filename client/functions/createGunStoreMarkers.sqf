...
_setStatus =
{
	...
	switch(_this select 1) do {
		case "EMPTY": {
			...
			_markerNameDescription setmarkerColorLocal "ColorYellow";
	...
	if (["A3W_gunStoreIntruderWarning"] call isConfigOn) then
	{
		if((_this select 2) && ((_this select 1) in ["ENEMY", "MIXED"])) then {
			hintSilent parseText format ["<t size='2' color='#ff0000'>%1</t><br/><br/>%2.","Aviso!","Jogador inimigo entrou na área"];
		};
	};

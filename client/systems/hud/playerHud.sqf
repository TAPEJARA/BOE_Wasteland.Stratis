	...
	switch (_territoryAction) do
	{
		case "CAPTURE":
		{
			...
			if (_territoryCaptureCountdown > 60) then
			{
				...
				_territoryActionText = format["Capturando território em aproximadamente <t font='%1'>%2 minutos</t>", _boldFont, _minutes + 1];
			}
			else
			{
				...
				{
					_territoryActionText = format["Capturando território em <t font='%1'>%2 secondos</t>", _boldFont, _territoryCaptureCountdown];
				};
			};
		};
		...
		case "RESET":           { _territoryActionText = "A captura do território começou" };
	};

	_activityMessage = format ["Local: <t font='%1'>%2</t><br/>%3", _boldFont, _descriptiveName, _territoryActionText];

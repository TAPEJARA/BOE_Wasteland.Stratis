...
_respawnText ctrlSetStructuredText parseText (format ["Bem-vindo ao BOE Wasteland<br/>Você está no %1. Por favor selecione um respawn.", _side]);
	...
	if (!isNil "_location") then
	{
		...
		else
		{
			if (_location != "") then
			{
				...
				{
					_textStr = _textStr + "[<t color='#ff0000'>Bloqueado pelo inimigo</t>] ";
				}
				else
				{
		...
		if (_isValid) then
		{
			...
			if (_friendlyPlayers > 0) then
			{
				...
				_extraTextStr = _extraTextStr + format ["<t color='#00ff00'>%1 amigo(s)</t>", _friendlyPlayers];
			};

			if (_enemyPlayers > 0) then
			{
				if (_extraTextStr != "") then { _extraTextStr = _extraTextStr + ", " };
				_extraTextStr = _extraTextStr + format ["<t color='#ff0000'>%1 inimigo(s)</t>", _enemyPlayers];
			};

			if (_enemyNPCs > 0) then
			{
				if (_extraTextStr != "") then { _extraTextStr = _extraTextStr + ", " };
				_extraTextStr = _extraTextStr + format ["<t color='#ff0000'>%1 BOT(s)</t>", _enemyNPCs];
			};
...
[_selLocChanged] spawn
{
	...
	while {!isNull _display} do
	{
		...
		_missionUptimeText ctrlSetText format ["Tempo da missão: %1", _timeText];

...
if (!isDedicated) then
{
	[] spawn
	{
		...
		{
			9999 cutText ["Bem vindo ao BOE A3Wasteland, por favor aguarde o carregamento para iniciar", "BLACK", 0.01];
...
//    [] execVM "addons\proving_ground\init.sqf";
	...
	[] execVM "addons\scripts\effect.sqf";
//	  [] execVM "restart.sqf";
	[] execVM "addons\scripts\push_init.sqf";		
	if (isServer) then {call compile preprocessFile "mapconfig\structures\initBuildings.sqf";}; // GID Positioning System

	/*CHBN_adjustBrightness = 90;
	CHBN_adjustColor = [0.2,0.4,1];
